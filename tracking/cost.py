...
def iou_2d(bboxes1: np.ndarray, bboxes2: np.ndarray) -> np.ndarray:
    ...
    for m in range(M):
        ...
        for n in range(N):
            ...
            di = distance(m_polygon,n_polygon)
            v = (4 / (math.pi ** 2)) * math.pow((np.arctan(h2 / w2) - np.arctan(h1 / w1)), 2)
            if(iou >= 0.5):
                alpha = (v/((1-iou)+v))
                iou_mat[m][n]= iou + di + (alpha * v)
            else:
                iou_mat[m][n] = iou + di
    return iou_mat
...
#         h1,w1,angle1 = bboxes1[i][2], bboxes1[i][3], bboxes1[i][4]
#         for j in range(0,N):
#             n = rotate_box(bboxes2[j])
#             h2,w2,angle2 = bboxes2[j][2], bboxes2[j][3], bboxes2[j][4]
#             iou = m.intersection(n).area / m.union(n).area
#             # di = distance(m,n)
#             # yaw_dis = math.sqrt(di + ((angle2 - angle1)**2))
#             # v = (4 / (math.pi ** 2)) * math.pow((np.arctan(h2 / w2) - np.arctan(h1 / w1)), 2)
#             # if(iou >= 0.5):
#             #     alpha = (v/((1-iou)+v))
#             #     iou_mat[i][j]= iou + di + (alpha * v)
#             # else:
#             # iou_mat[i][j]= iou + di + yaw_dis
#             iou_mat[i][j] = iou
#     return iou_mat
